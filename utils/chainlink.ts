...
import { RequestParamType, chainIdLink } from "./chainlink-constants";
  ...
  for (const { type, name, value, valueTypes } of requestParams) {
    switch (type) {
      ...
      case RequestParamType.BYTES:
        ...
        break;
      case RequestParamType.BYTES_PACKED:
        if (!Array.isArray(value)) {
          throw new Error(
            `Request param '${name}' of type '${type}' does not have and Array for 'value': ${JSON.stringify(value)}`,
          );
        ...
        await toolsChainlinkTestHelper.addBytes(name, ethers.utils.solidityPack(valueTypes, value));
        break;
      case RequestParamType.BYTES_RAW:
        if (!ethers.utils.isHexString(value)) {
          throw new Error(
            `Request param '${name}' of type '${type}' is not a valid hex string: ${JSON.stringify(value)}`,
          );
      ...
      case RequestParamType.UINT:
        ...
        break;
      // NB: EXPERIMENTAL, implemented by LinkPool
...
export async function convertRequestParamsToCborBufferExperimental(
  ...
  for (const { type, name, value } of requestParams) {
    ...
    if (new Set([RequestParamType.STRING_ARRAY, RequestParamType.UINT_ARRAY, RequestParamType.INT_ARRAY]).has(type)) {
      buffer += "ff";
    }
  }
...
export async function getLinkTokenContract(
  ...
  return new hre.ethers.Contract(addressLINK, linkTokenArtifact.abi, signer) as LinkToken;
}
...
export async function getLinkBalanceOf(
  ...
  const contractLINK = await getLinkTokenContract(hre, signer, addressLINK);
