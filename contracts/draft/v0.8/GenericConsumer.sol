...
contract GenericConsumer is ConfirmedOwner, Pausable, KeeperBase, TypeAndVersionInterface, IGenericConsumer {
    ...
    function performUpkeep(bytes calldata _performData) external override whenNotPaused {
        ...
        for (uint256 i = 0; i < keysLength; ) {
            ...
            _requireLinkBalanceIsSufficient(consumer, consumerLinkBalance, entry.payment);
        ...
        s_consumerToLinkBalance[consumer] = consumerLinkBalance;
    ...
    function checkUpkeep(bytes calldata _checkData) external view override returns (bool, bytes memory) {
        (uint256 lot, address consumer) = abi.decode(_checkData, (uint256, address));
        _requireLotIsInserted(lot, s_lotToEntryMap.isInserted(lot));
        EntryLibrary.Map storage s_keyToEntry = s_lotToEntryMap.getLot(lot);
        ...
        if (noEntriesToRequest > 0 && s_consumerToLinkBalance[consumer] >= roundPayment) {
            isUpkeepNeeded = true;
