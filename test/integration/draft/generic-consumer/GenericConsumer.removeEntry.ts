...
export function testRemoveEntry(signers: Signers, context: Context): void {
  ...
  it("reverts when the caller is not the owner", async function () {
    ...
    // Act & Assert
    await expect(
      context.genericConsumer.connect(signers.externalCaller).removeEntry(lot, key, overrides),
  ...
  it("does not revert when GenericConsumer is paused", async function () {
    ...
    // Act & Assert
    await expect(
      context.genericConsumer.connect(signers.owner).removeEntry(lot, key, overrides),
  ...
  it(`reverts when the lot is not inserted`, async function () {
    ...
    // Act & Assert
    await expect(context.genericConsumer.connect(signers.owner).removeEntry(lot, key, overrides)).to.be.revertedWith(
  ...
  it(`reverts when the Entry is not inserted`, async function () {
    ...
    // Act & Assert
    await expect(context.genericConsumer.connect(signers.owner).removeEntry(lot, key1, overrides)).to.be.revertedWith(
      `GenericConsumer__EntryIsNotInserted(${lot}, "${key1}")`,
    );
  });

  it(`removes an Entry (lot is preserved)`, async function () {
    // Arrange
    const noEntries = 2;
    const entries = parseEntriesFile(path.join(filePath, "file1.json"));
    ...
    await context.genericConsumer
      .connect(signers.owner)
      .setLastRequestTimestamps(lot, [key0, key1], [BigNumber.from("1652354650"), BigNumber.from("1652354651")]);
    await context.genericConsumer.connect(signers.owner).setIsUpkeepAllowed(lot, true);

    // Act & Assert
    await expect(context.genericConsumer.connect(signers.owner).removeEntry(lot, key0, overrides))
    ...
    // await expect(context.genericConsumer.connect(signers.defaultAdmin).getEntry(lot, key0)).to.be.revertedWith(
    //   `GenericConsumer__EntryIsNotInserted(${lot}, \\"${key0}\\")`,
    // );
    // await expect(context.genericConsumer.getLastRequestTimestamp(lot, key0)).to.be.revertedWith(
    //   `GenericConsumer__EntryIsNotInserted(${lot}, \\"${key0}\\")`,
    // );
    expect(await context.genericConsumer.getEntryIsInserted(lot, key1)).to.be.true;
    expect(await context.genericConsumer.getIsUpkeepAllowed(lot)).to.be.true;
  });

  it(`removes an Entry (lot is removed)`, async function () {
    // Arrange
    const entries = parseEntriesFile(path.join(filePath, "file1.json"));
    await setCodeOnEntryContractAddresses(hardhat, entries);
    const entryConvertedMap = await getEntryConvertedMap(entries, context.toolsChainlinkTestHelper);
    ...
    // Act & Assert
    await expect(context.genericConsumer.connect(signers.owner).removeEntry(lot, key, overrides))
      .to.emit(context.genericConsumer, "EntryRemoved")
      .withArgs(lot, key);
    expect(await context.genericConsumer.getLotIsInserted(lot)).to.be.false;
    // TODO: amend once hardhat fixes issue https://github.com/ethers-io/ethers.js/discussions/2849
    await expect(context.genericConsumer.connect(signers.owner).getEntry(lot, key)).to.be.revertedWith(
      `GenericConsumer__LotIsNotInserted(uint256)`,
    );
    await expect(context.genericConsumer.getLastRequestTimestamp(lot, key)).to.be.revertedWith(
      `GenericConsumer__LotIsNotInserted(uint256)`,
    );
  ...
  it(`performs the cleaning tasks after removing the lot (maps don't restore the old values)`, async function () {
    ...
    await context.genericConsumer.connect(signers.owner).removeEntry(lot, key, overrides);
    await context.genericConsumer.connect(signers.owner).setEntry(lot, key, entryConverted, overrides);
